...
if ( PHP_INT_SIZE == 4 ) // x86
{
	echo "The XPath 2.0 conformance tests cannot succeed when running x86 version of PHP because " .
		 "some conformance tests will only succeed when an 64 bit integer is available.";
	return;
}
...
/*  768 */ XQSTests::runTestGroups( "/MinimalConformance/Expressions/Operators/ArithExpr/NumericOpr", function( $test, $file, $first ) { return true || $test == 'K-NumericIntegerDivide-32' || ! $first; } ); terminate( $terminate );
